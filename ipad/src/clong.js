...
screenSize[ 1 ] = screenSize[ 1 ] > 600 ? screenSize[ 1 ] : 600;

var configs = {
  containerSize: [ screenSize[ 0 ], screenSize[ 1 ] ],
  desktopStartPosition: [ 0.12 * screenSize[ 0 ], 0.043 * screenSize[ 1 ] ],
  desktopSpaceSize: [ 0.14 * screenSize[ 0 ], 0.186 * screenSize[ 1 ] ],
  desktopSize: [ screenSize[ 0 ], screenSize[ 1 ] * 0.83 ],
  iconSize: [ 0.056 * screenSize[ 0 ], 0.056 * screenSize[ 0 ] ],
  dockStartPosition: [ 0.188 * screenSize[ 0 ], screenSize[ 1 ] * 0.067 ],
  dockSpaceSize: [ 0.14 * screenSize[ 0 ], 0.067 * screenSize[ 1 ] ],
  dockContainerSize: [ screenSize[ 0 ], screenSize[ 1 ] * 0.23 ],
  dockSize: [ screenSize[ 0 ], 0.17 * screenSize[ 1 ] ],
  colNum: 6
}
...
let css = {
  ...
  reduce: ( desktop, element ) =>{
    ...
    desktopContainer.style.height = cofigs.desktopSize[ 1 ] + 'px';
...
var sliderDesktopIndexEndPositionRender = () =>{
  Array.from( sliderDesktop.children ).forEach( ( item, index ) => {
    Array.from( item.children ).forEach( ( el, index ) => {
      ...
      css.size( el, [ configs.iconSize[ 0 ], configs.iconSize[ 1 ] ] );
      css.size( el.firstChild, [ configs.iconSize[ 0 ], configs.iconSize[ 1 ] ] );
      css.transform( el, [ 
